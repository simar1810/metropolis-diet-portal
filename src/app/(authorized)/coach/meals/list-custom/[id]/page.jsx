...
function MealPlanDetailsContainer({ id }) {
  const { isLoading, error, data } = useSWR(`custom-meal-plans/${id}`, () =>
    getCustomMealPlans("coach", id),
  );

  const responseData = data?.data;
  const hasNoPlan = Array.isArray(responseData)
    ? responseData.length === 0
    : !responseData;
  const customPlan = Array.isArray(responseData)
    ? responseData[0]
    : responseData;
  ...
  useEffect(() => {
    ...
    const hasSelected = mealsForPlan.some(
  ...
  if (error || data?.status_code !== 200 || hasNoPlan) {
    return (
      <ContentError title={error || data?.message || "No Such Plan Found!"} />
    );
  }
  ...
  return (
    <main>
      ...
      <DisplayMealStats
        targetTotals={
          customPlan.aiResponseRaw?.plan?.daily?.totals ||
          customPlan.aiResponseRaw?.plan?.daily?.totals ||
          {}
        }
        meals={{ plans: { [selectedPlan]: customPlan.plans[selectedPlan] } }}
      />
      <div className="content-container content-height-screen mt-4">
        <CustomMealMetaData
...
function CustomMealMetaData({ customPlan }) {
  ...
  const handleSendPdf = async function () {
    ...
    try {
      ...
      setPdfFile(
        new File([blob], "meal-plan.pdf", { type: "application/pdf" }),
      );
      setPdfSent(true);
  ...
  return (
    <div className="p-4 pr-8">
      <div className="flex items-center gap-4">
        ...
        {pdfSent && (
          <Button onClick={downloadPDF} variant="icon">
            <Download />
          </Button>
        )}
        {!pdfSent && (
      ...
      <div className="flex items-center gap-2">
        {!isNaN(customPlan.noOfDays) && (
          <div className="font-bold">{customPlan.noOfDays} Days</div>
        )}
      </div>
      {/*<Image
        alt=""
...
function CustomMealsListing({
  ...
  const planForDay = selectedPlan
    ? customPlan.plans?.[selectedPlan]
    : undefined;
  ...
  const selectedMealsForMealType =
    selectedMealTypes.find((type) => type?.mealType === selectedMealType)
      ?.meals || [];

  const normalizedSelectedMealsForMealType = useMemo(() => {
    const shouldFlatten = ["lunch", "dinner"].includes(
      String(selectedMealType || "").toLowerCase(),
    );
  ...
  return (
    <div className="p-4 md:pl-8 relative">
      {customPlan.draft && (
        ...
        <div className="mt-6 p-4 rounded-xl bg-gray-50/50 border border-gray-100 grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div className="flex flex-col">
            ...
            <span className="text-lg font-bold text-[#1A1A1A] leading-tight">
              {mealTypeTotals.calories.toFixed(1)}{" "}
          ...
          <div className="flex flex-col">
            ...
            <span className="text-lg font-bold text-[#1A1A1A] leading-tight">
              {mealTypeTotals.protein.toFixed(1)}{" "}
              <span className="text-[12px] font-normal text-gray-500">g</span>
            </span>
          </div>
          <div className="flex flex-col">
            <span className="text-[11px] text-gray-400 font-bold uppercase tracking-wider">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-lg font-bold text-[#1A1A1A] leading-tight">
              {mealTypeTotals.carbohydrates.toFixed(1)}{" "}
              <span className="text-[12px] font-normal text-gray-500">g</span>
            </span>
          </div>
        </div>
      ...
      <div className="mt-auto pt-4 border-t border-gray-50">
        <div className="grid grid-cols-3 gap-y-4 gap-x-2">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[13px] font-semibold text-[#1A1A1A] truncate">
              {meal.carbohydrates || meal.carbs} g
            </span>
          </div>
          <div className="flex flex-col">
            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">
...
export function DeleteCustomMealPlan({ id }) {
  ...
  return (
    <DualOptionActionModal
      ...
      action={(setLoading, closeBtnRef) =>
        deleteCustomPlan(setLoading, closeBtnRef)
      }
    >
      <AlertDialogTrigger>
  ...
  return (
    <div className="bg-white px-4 py-3 border-b sticky top-0 z-10">
      <div className="max-w-7xl mx-auto flex flex-wrap gap-x-8 gap-y-2 items-center text-sm">
        <span className="font-bold text-gray-500 uppercase text-[10px] tracking-widest mr-2">
          Total Daily Intake:
        </span>
  ...
  return (
    <div className="mt-6">
      <div className="space-y-8">
        {selectedMealsForMealType.map((type, idx) => (
          <MealTypeListView type={type} key={type.optionType || idx} />
        ))}
      </div>
    </div>
  );
  ...
  return (
    <div className="mb-10 last:mb-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
        ...
        {meals?.length === 0 && (
          <div className="h-[200px] bg-gray-50 rounded-xl flex items-center justify-center border border-dashed border-gray-200 text-gray-400 col-span-full">
...
function SendPDFViaWhatsapp({ clientId, pdf }) {
  ...
  async function sendMealPlanPdfViaWhatsapp(setLoading) {
    const toastId = toast.loading(
      "The meal plan creation and delivery process is currently in progress",
    );
    try {
      setLoading(true);
      const formData = {
      ...
      const response = await sendData(
        "app/metropolis/whatsapp/generate-share-pdf",
        formData,
      );
      if (response.status_code !== 200) throw new Error(response.message);
  ...
  return (
    <DualOptionActionModal
      ...
      action={(setLoading, btnRef) =>
        sendMealPlanPdfViaWhatsapp(setLoading, btnRef)
      }
    >
      <AlertDialogTrigger>
